...
class BeneficiarioController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // NOVO: filtro de recibos ausentes
        if ($request->filled('recibo')) {
            if ($request->recibo === 'rec12_ausente') {
                $query->where(function ($q) {
                    $q->whereNull('rec1')->orWhere('rec1', 0);
                })->where(function ($q) {
                    $q->whereNull('rec2')->orWhere('rec2', 0);
                });
            } elseif ($request->recibo === 'rec34_ausente') {
                $query->where(function ($q) {
                    $q->whereNull('rec3')->orWhere('rec3', 0);
                })->where(function ($q) {
                    $q->whereNull('rec4')->orWhere('rec4', 0);
                });
            }
        }
    ...
    public function sugestoes(Request $request)
    {
        ...
        if ($request->filled('recibo')) {
            if ($request->recibo === 'rec12_ausente') {
                $query->where(function ($q) { $q->whereNull('rec1')->orWhere('rec1', 0); })
                      ->where(function ($q) { $q->whereNull('rec2')->orWhere('rec2', 0); });
            } elseif ($request->recibo === 'rec34_ausente') {
                $query->where(function ($q) { $q->whereNull('rec3')->orWhere('rec3', 0); })
                      ->where(function ($q) { $q->whereNull('rec4')->orWhere('rec4', 0); });
            }
        }
    ...
    {
        // NOVO: 'recibo' incluído na lista de filtros exportados
        $filtros     = $request->only(['nome', 'social_id', 'municipio', 'bairro', 'pago', 'sexo', 'recibo']);
